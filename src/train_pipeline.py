...
def run_training(tol,epsilon):
    ...
    while True:
      ...
      mse = 0
      for batch in range(0,X_train.shape[0],2):
        X_batch = X_train[batch:batch+config.MINI_BATCH_SIZE]
        Y_batch = Y_train[batch:batch+config.MINI_BATCH_SIZE]
        
        batch_del_theta0 = [np.zeros_like(theta0) for theta0 in pl.theta0]
        batch_del_theta = [np.zeros_like(theta) for theta in pl.theta]
        ...
        L = (1/2)*(Y_batch - h[config.NUM_LAYERS-1])**2
      ...
      mse = np.mean(mse/X_batch.shape[0])
      epoch_counter = epoch_counter + 1
      loss_per_epoch.append(mse)
      print("Epoch # {}, Loss = {}".format(epoch_counter,mse))

      if(abs(mse<tol)or epoch_counter>=max_epoch ):
        break

if __name__ == "__main__":
   run_training(10**(-4),10**(-7))
